...
- 每张快讯卡最多包含 3 条短新闻。
- 每轮默认最多生成 `12` 张图片。
- 分类按固定顺序展示：AI、芯片、公司、市场、国际、政策、科研、其他。
- 分类颜色固定，不再按页码变化。
...
系统会先让 DeepSeek 总结单条新闻，再按信息量排版：

- `keyPoints >= 7` 或 `keyPoints` 总字数 `>= 180`：允许生成单新闻卡。
- `keyPoints <= 4` 或 `keyPoints` 总字数 `< 140`：进入快讯池。
- `keyPoints` 为 5-6 条时，只有总信息量足够才生成单新闻卡，否则进入快讯池。

快讯池按分类分组，每张快讯卡最多 3 条新闻。分类内排序为：单新闻卡、3 条快讯卡、2 条快讯卡、1 条快讯卡。

## 规则去重

新闻进入 DeepSeek 总结前，会先做规则去重，减少同一事件因多个 RSS 源转载而重复成卡：

- URL 规范化后相同会合并。
- RSS `guid/id` 相同会合并。
- 标题去掉来源后缀、标点和多余空格后相同会合并。
- 标题 token overlap 或中英文 ngram 相似度超过阈值会合并。

合并时优先保留来源质量更高、标题更完整、有摘要、发布时间更新的新闻。Actions 日志会打印原始新闻数、去重后新闻数、被合并数量，以及前若干组重复标题，便于调试阈值。
